...
# ./script_C++.sh "nbody.py 50000000" "nbody_50000"
# ./script_C++.sh "<c++_file arguments>" "<path or folder or directory>"
...
# MENU
echo "Available GCC  versions are the next ones:"
for version in "${versions[@]}"; do
  echo "  - $version"
done
echo -n "  - all

 *** Type the version (for example 'gcc-13', or 'all' to measure all Python versions): "
read version

# CONDITION
if [ $version == 'all' ] || [ $version == 'All' ] || [ $version == 'ALL' ]; then
  ./script_C++_all.sh "$1" "$2"
else
