...
# ./script_Python_all.sh "nbody.py 50000000" "nbody_50000000"
# ./script_Python_all.sh "<python_file arguments>" "<path or folder or directory>"
...
echo "Running script_Python_all.sh $1 $2"

# Output file for recording turbostat data in CSV format
program=$(echo $1 | awk -F. '{print $1}')
path_folder=python/$2
[ -d $path ] || mkdir $path
output_generalFile="$path_folder/performance_data_allVersions_$program.csv"
temp_file="$path_folder/temp_performance_data.txt"
